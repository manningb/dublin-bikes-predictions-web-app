...
def get_data(data, station_number):
    ...
    return (station_number, now, datetime.datetime.fromtimestamp(data.get("dt")), data.get("main").get("temp"), data.get("main").get("feels_like"), data.get("main").get("pressure"), data.get("main").get("humidity"), data.get("visibility"), data.get("wind").get("speed"), data.get("wind").get("deg"), data.get("weather")[0].get("main"), data.get("weather")[0].get("description"))
...
def loop_through_2020_by_week(station_number, station_lng, station_lat, start_week, end_week):
    ...
    for i in range(start_week, end_week):
        ...
        for day in days:
            print(day)
            vals = get_data(day, station_number)
            engine.execute("INSERT INTO `dublin_bikes`.`weather_historical_daily` values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",vals)
            print(vals)
